...
if __name__=="__main__":
    ...
    c=conn.cursor()
    while True:
        c.execute("SELECT string FROM tmp_new_unique_strings LIMIT %d,%d"%(OFFSET,BATCH))
        rows=c.fetchall()
        OFFSET+=BATCH
        if len(rows)==0:
            break
        for (symbol,) in rows:
            try:
                (pref,symb,suff), form=stripSymbol(symbol,symbDBs,orgDB,prefixDB,suffixDB,prefixCounts,suffixCounts)
            except:
                symb=symbol
