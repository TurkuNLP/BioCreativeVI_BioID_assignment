...
def tiss_combine_matching(predict_list, anno_dict):
    """
    ...
    for doc_id, annos in anno_dict.iteritems():
        for i, anno in enumerate(annos):
            ...
            if all_predict != {}:
                ...
                combine_dict.setdefault(doc_id, []).append([anno[0], anno[1], anno[2], 'tiss', 'Uberon:' + predict_id])
                norm_tis_temp.setdefault(doc_id, []).append([anno[0], anno[1], anno[2], predict_id, 'tiss', max_t])
            else:
                combine_dict.setdefault(doc_id, []).append([anno[0], anno[1], anno[2], 'tiss', 'tissue:' + anno[2]])
                norm_tis_temp.setdefault(doc_id, []).append([anno[0], anno[1], anno[2], '', 'tiss', max_t])        
    return norm_tis_temp, combine_dict


def load_tissue_mapping(in_file, col_1, col_2, normform):
    map_dict = dict()
...
def tiss_match_simstring(ss_folder, annoDict, ab3p_dict, ms, mapping_dict, db_file, normform, min_threshold=0.01):
...
def normalize_tis(anno_dict, ab3p_dict):
    ss_folder = os.getcwd() + '/simstring/'
    in_dict = load_tissue_mapping(ss_folder + 'tissue.tsv.gz', 0, 1, False)
    db_file = 'tissue/tissue.db'
    predict_list = []
    predict_list.append(tiss_match_simstring(ss_folder, anno_dict, ab3p_dict, simstring.cosine, in_dict, db_file, False, 0.01))
    norm_tis_temp, combine_dict = tiss_combine_matching(predict_list, anno_dict)
    return norm_tis_temp, combine_dict
