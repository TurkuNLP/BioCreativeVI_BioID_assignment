...
def argument_parser():
    ...
    parser.add_argument("-f", "--filename", type=str, default='solr/temp_final_gene.tsv.gz', help="input file to be put into solr")
    ...
    parser.add_argument("-a", "--solr_address", default="http://localhost:8983/solr/chemicals", type=str, help="solr core address where gene protein mapping ")
...
if __name__ == "__main__":
    ...
    try:
        quotes = pickle.load(gzip.open(os.path.join(os.path.dirname(__file__), args.filename)))
        # quotes = pickle.load(open(args.filename.replace('.tsv.gz', '_list.pkl'), 'rb'))
        print 'load pickle file'
    except:
        print 'no list file, should re-process'
        print os.path.join(os.path.dirname(__file__), args.filename)
        quotes = get_gene(os.path.join(os.path.dirname(os.path.abspath(__file__)), args.filename))
    for item in quotes:
        print item
    # clean_up(quotes, args.filename) # "http://localhost:8983/solr/chemicals"
